           ...
           @forelse($listJawaban as $items)
               <div class="list-people border rounded-1 p-2 d-flex flex-wrap justify-content-between mb-3" style="margin-bottom: -10px">
                   <div class="identity d-flex flex-wrap">
                       ...
                       <div>
                           <a class="d-block mb-3 btn btn-sm btn-primary ms-3 mt-1" style="margin-bottom: 6px !important;"
                              href="{{ route('student.download.jawaban.ujian', ['id_jawaban' => $items->id]) }}">
                   ...
                   <div class="action-siswa">
                       <form action="{{ route('teacher.nilai.ujian.post') }}" class="d-flex flex-wrap justify-content-end" method="post">
                           @csrf
                           <input type="hidden" name="id_kelas" value="{{ $items->kelas_id }}" style="display: none;">
                           <input type="hidden" name="id_ujian" value="{{ $items->ujian_id }}" style="display: none;">
                           <input type="hidden" name="id_siswa" value="{{ $items->siswa_id }}" style="display: none;">
